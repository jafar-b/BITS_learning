...
async function displayProductsList() {
  const initialProductsArray = await fetchProductsList();

  const mid = Math.floor(initialProductsArray.length / 2);
  const slicedArray1 = initialProductsArray.slice(0, mid);
  const slicedArray2 = initialProductsArray.slice(mid);
...
async function setAllElements(productsArray) {
